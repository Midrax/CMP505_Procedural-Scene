	...
	for (UINT i = 0; i < mat->GetTextureCount(type); i++) {
		...
		if (!skip) {   // If texture hasn't been loaded already, load it
			...
			if (textype == "embedded compressed texture") {
				...
				if (FAILED(hr))
					MessageBox(hwnd_, (LPCWSTR)"Texture couldn't be loaded", (LPCWSTR)"Error!", MB_ICONERROR | MB_OK);
			}
...
ID3D11ShaderResourceView * ModelLoader::getTextureFromModel(const aiScene * scene, int textureindex) {
	...
	if (FAILED(hr))
		MessageBox(hwnd_, (LPCWSTR)"Texture couldn't be created from memory!", (LPCWSTR)"Error!", MB_ICONERROR | MB_OK);
